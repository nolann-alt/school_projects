...
SELECT Pilote.nomPilote, Compagnie.nomComp
FROM Pilote, Compagnie
WHERE compPil = Compagnie.idComp
/* 6 tuples
# nomPilote, nomComp
'Ridard', 'Air France'
...
SELECT Pilote.nomPilote, Compagnie.nomComp
FROM Pilote
    JOIN Compagnie ON compPil = Compagnie.idComp
/* 6 tuples
# nomPilote, nomComp
'Ridard', 'Air France'
'Fleurquin', 'Air France'
 ...
 */
 ...
 -- Question 3 :
/* 1) Quels sont les compagnies ‘low cost’ ? */
SELECT *
FROM Compagnie
WHERE estLowCost = '1' ;
/* 2 tuples
...
/* 2) Quels sont les noms des pilotes n’ayant pas de qualification ? */
SELECT DISTINCT UPPER(Pilote.nomPilote)
FROM Pilote
EXCEPT
SELECT DISTINCT UPPER(Pilote.nomPilote)
FROM Pilote, Qualification
WHERE idPilote = Qualification.unPilote ;
/* 1 tuple
# UPPER(Pilote.nomPilote)
'KERBELLEC'
 */

 /* 3) Quels sont les noms des 5 premiers pilotes tri´es par ordre alphab´etique d´ecroissant ? */
...
SELECT DISTINCT UPPER(nomEns), UPPER(prenomEns)
FROM Enseignant
WHERE idEns IN (SELECT DISTINCT tuteurGroupe
                FROM GroupeInfo1);
/* 4 tuples
...
WHERE idEns IN (SELECT DISTINCT tuteurGroupe
                FROM GroupeInfo1
                WHERE idEns IN (SELECT DISTINCT tuteurApp
                                FROM Apprenti));
/* 2 tuples
# UPPER(nomEns)
'TUFFIGO'
...
FROM Entreprise
WHERE idEntreprise IN (SELECT DISTINCT entrepriseApp
                       FROM Apprenti
                       WHERE tuteurApp = 'MM');
/* 6 tuples
...
WHERE idEns IN (SELECT DISTINCT tuteurApp
                ...
                WHERE entrepriseApp IN (SELECT DISTINCT idEntreprise
                                        FROM Entreprise
                                        WHERE depEntreprise NOT IN (35, 29, 56, 22)));
/* 2 tuples
...
-- Question 9
/* Pour chaque stagiaire (id), afficher le nom et le d´epartement de l’entreprise. */
SELECT DISTINCT etudStagiaire, UPPER(nomEntreprise), UPPER(depEntreprise)
FROM Stagiaire
    JOIN Entreprise ON entrepriseStage = Entreprise.idEntreprise;
/* 58 tuples
# etudStagiaire, UPPER(nomEntreprise), UPPER(depEntreprise)
 ...
 */
 ...
 -- Question 10 :
/* Pour chaque stagiaire (nom et pr´enom), afficher le nom et le d´epartement de l’entreprise.*/
SELECT DISTINCT UPPER(Etudiant.nomEtud), UPPER(Etudiant.prenomEtud), UPPER(nomEntreprise), UPPER(depEntreprise)
FROM Stagiaire, Etudiant, Entreprise
WHERE idEtud = Stagiaire.etudStagiaire
AND entrepriseStage = Entreprise.idEntreprise;
/* 58 tuples
# UPPER(Etudiant.nomEtud), UPPER(Etudiant.prenomEtud), UPPER(nomEntreprise), UPPER(depEntreprise)
...
-- Question 11 :
/* Afficher les ´etudiants (nom et pr´enom) ayant r´ealis´e un stage dans un d´epartement diff´erent de celui du lyc´ee. */
SELECT DISTINCT UPPER(Etudiant.nomEtud), UPPER(Etudiant.prenomEtud)
FROM Etudiant
    JOIN Stagiaire ON idEtud = Stagiaire.etudStagiaire
        JOIN Entreprise ON entrepriseStage = Entreprise.idEntreprise
...
-- Question 12 :
/* Pour chaque enseignant (nom), afficher ses apprentis (nom et pr´enom), si ils existent et rien sinon. */
SELECT DISTINCT nomEns, Etudiant.nomEtud, Etudiant.prenomEtud
FROM Enseignant
    LEFT JOIN Apprenti ON tuteurApp = Enseignant.idEns
        LEFT JOIN Etudiant ON idEtud = Apprenti.etudApp ;
/* 38 tuples
 ...
 */
-- Question 13 :
/* Pour chaque ´etudiant (nom), afficher son entreprise (nom) si elle existe et rien sinon. */
SELECT DISTINCT UPPER(nomEtud), UPPER(nomEntreprise)
FROM Etudiant
    LEFT JOIN Apprenti ON etudApp = Etudiant.idEtud
        LEFT JOIN Stagiaire ON idEtud = etudStagiaire
            LEFT JOIN Entreprise ON (idEntreprise = Apprenti.entrepriseApp OR idEntreprise = Stagiaire.entrepriseStage) ;
/* 107 tuples
# UPPER(nomEtud), UPPER(nomEntreprise)
'VIAUD', 'SCM VIAUD-FORMAGNE'
'JOSSE', 'CGI'
'LEBORGNE', 'ANVERGUR'
'RECOLIN', 'ACCENTURE'
'GUENNEC', 'IRISA, EQUIPE OBELIX'

 */

 -- Question 14 :
/* Pour chaque enseignant (nom), afficher les apprentis (nom et pr´enom) et l’entreprise (nom), s’ils existent et rien sinon. */
SELECT DISTINCT UPPER(nomEns), UPPER(Etudiant.nomEtud), UPPER(Etudiant.prenomEtud), UPPER(nomEntreprise)
FROM Enseignant
    LEFT JOIN Apprenti ON tuteurApp = Enseignant.idEns
        LEFT JOIN Etudiant ON idEtud = Apprenti.etudApp
            LEFT JOIN Entreprise ON idEntreprise = Apprenti.entrepriseApp ;
/* 38 tuples
# UPPER(nomEns), UPPER(Etudiant.nomEtud), UPPER(Etudiant.prenomEtud), UPPER(nomEntreprise)
 ...
 */
 ...
 -- Question 15 :
/* Pour chaque enseignant (nom), afficher les apprentis (nom et pr´enom) en Bretagne et le d´epartement, tri´es dans l’ordre d´ecroissant du d´epartement, s’ils existent et rien sinon. */
SELECT DISTINCT UPPER(nomEns), UPPER(Etudiant.nomEtud), UPPER(Etudiant.prenomEtud), UPPER(Entreprise.depEntreprise)
FROM Enseignant
    LEFT JOIN Apprenti ON tuteurApp = Enseignant.idEns
        LEFT JOIN Etudiant ON idEtud = Apprenti.etudApp
            LEFT JOIN Entreprise ON idEntreprise = Apprenti.entrepriseApp
WHERE UPPER(depEntreprise) IN (35, 29, 56, 22) OR depEntreprise IS NULL
ORDER BY UPPER(depEntreprise) DESC ;
/* 36 tuples
# UPPER(nomEns), UPPER(Etudiant.nomEtud), UPPER(Etudiant.prenomEtud), UPPER(Entreprise.depEntreprise)
